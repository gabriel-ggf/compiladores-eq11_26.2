...
|:------:|:----:|-----------|-----------|
| `0.1.0` | 26/08/2026 | Estruturação inicial do projeto, pipeline Flex/Bison em C e documentação base. | Equipe 11 |
...
3. O código do repositório pode continuar em `C:\...` e ser acessado de dentro do WSL em `/mnt/c/...`, ou ser clonado direto dentro do filesystem do WSL (mais rápido para compilar) — qualquer uma das duas funciona.
...
O time segue **Scrum**, com sprints semanais. Datas conforme o Plano de Ensino (FGA0003 — Compiladores 1, T02, 2026/2):
...
> Datas de apresentação assumindo que somos a **Equipe 11** na numeração do professor (1 a 16) — confirmem isso com o professor/turma.

**Não enviar o formulário no prazo = nota 0 na entrega.** Só o líder da equipe deve enviar; presença de todos os membros é obrigatória nas apresentações/entrevistas de P1, P2 e T.
...
- `C` = código entregue (correção, funcionalidades, organização, documentação) — **maior peso**.
- `T` = entrevista final (explicação técnica + justificativa das decisões).
